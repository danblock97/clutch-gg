...
export async function GET(request) {
  try {
    ...
    const bat = [
      ...
      debug ? "set DEBUG=1" : "set DEBUG=0",
      "",
      "REM Build alternate host (swap 80<->8088 to try both)",
      "for /f \"tokens=1,2 delims=:\" %%a in (\"%SPEC_HOST%\") do (set SPEC_HOST_NAME=%%a& set SPEC_HOST_PORT=%%b)",
      "if \"%SPEC_HOST_PORT%\"==\"8088\" (set SPEC_ALT_HOST=%SPEC_HOST_NAME%:80) else (set SPEC_ALT_HOST=%SPEC_HOST_NAME%:8088)",
      "",
      "REM Count existing LoL processes to avoid false positives",
      "for /f \"delims=\" %%C in ('tasklist /FI \"IMAGENAME eq League of Legends.exe\" ^| find /C \"League of Legends.exe\"') do set PRE_COUNT=%%C",
      "",
      "echo [Clutch.GG] Preparing spectator...",
      "echo [Clutch.GG] Region=%SPEC_PLATFORM% Game=%SPEC_GAME%",
      "REM ================= MetaTFT-style launch (pvp.net:8080) =================",
      ...
      "REM Do not modify Vanguard services; not required for spectator",
      "echo [Clutch.GG] Launching...",
      "if \"%DEBUG%\"==\"1\" echo CMD: \"League of Legends.exe\" \"spectator %PVP_HOST1% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\" -UseRads -GameBaseDir=.. \"-Locale=%LOCALE%\" -SkipBuild -EnableCrashpad=true -EnableLNP",
      "start \"\" \"League of Legends.exe\" \"spectator %PVP_HOST1% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\" -UseRads -GameBaseDir=.. \"-Locale=%LOCALE%\" -SkipBuild -EnableCrashpad=true -EnableLNP",
      "timeout /t 15 /nobreak >nul",
      "for /f \"delims=\" %%C in ('tasklist /FI \"IMAGENAME eq League of Legends.exe\" ^| find /C \"League of Legends.exe\"') do set POST_COUNT=%%C",
      "if NOT \"%POST_COUNT%\"==\"%PRE_COUNT%\" goto END_LAUNCHED",
      "if \"%DEBUG%\"==\"1\" echo [Clutch.GG] Trying alternate port...",
      "if \"%DEBUG%\"==\"1\" echo CMD: \"League of Legends.exe\" \"spectator %PVP_HOST2% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\" -UseRads -GameBaseDir=.. \"-Locale=%LOCALE%\" -SkipBuild -EnableCrashpad=true -EnableLNP",
      "start \"\" \"League of Legends.exe\" \"spectator %PVP_HOST2% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\" -UseRads -GameBaseDir=.. \"-Locale=%LOCALE%\" -SkipBuild -EnableCrashpad=true -EnableLNP",
      "timeout /t 15 /nobreak >nul",
      "for /f \"delims=\" %%C in ('tasklist /FI \"IMAGENAME eq League of Legends.exe\" ^| find /C \"League of Legends.exe\"') do set POST_COUNT=%%C",
      "if NOT \"%POST_COUNT%\"==\"%PRE_COUNT%\" goto END_LAUNCHED",
      "if \"%DEBUG%\"==\"1\" echo [Clutch.GG] Trying legacy launch...",
      "if \"%DEBUG%\"==\"1\" echo CMD: \"League of Legends.exe\" \"8394\" \"LoLLauncher.exe\" \"\" \"spectator %PVP_HOST1% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\"",
      "start \"\" \"League of Legends.exe\" \"8394\" \"LoLLauncher.exe\" \"\" \"spectator %PVP_HOST1% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\"",
      "timeout /t 8 /nobreak >nul",
      "for /f \"delims=\" %%C in ('tasklist /FI \"IMAGENAME eq League of Legends.exe\" ^| find /C \"League of Legends.exe\"') do set POST_COUNT=%%C",
      "if NOT \"%POST_COUNT%\"==\"%PRE_COUNT%\" (popd & goto END_LAUNCHED)",
      "if \"%DEBUG%\"==\"1\" echo CMD: \"League of Legends.exe\" \"8394\" \"LoLLauncher.exe\" \"\" \"spectator %PVP_HOST2% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\"",
      "start \"\" \"League of Legends.exe\" \"8394\" \"LoLLauncher.exe\" \"\" \"spectator %PVP_HOST2% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\"",
      "timeout /t 8 /nobreak >nul",
      "for /f \"delims=\" %%C in ('tasklist /FI \"IMAGENAME eq League of Legends.exe\" ^| find /C \"League of Legends.exe\"') do set POST_COUNT=%%C",
      "if NOT \"%POST_COUNT%\"==\"%PRE_COUNT%\" (popd & goto END_LAUNCHED)",
      "popd",
      ...
      "goto END",
      "",
      "REM ================= Try 1: Launch the game executable directly (no client UI) =================",
      "if defined GAME_EXE (",
      "  echo [Clutch.GG] Launching direct game spectator...",
      "  pushd \"%LL_DIR%\\Game\"",
      "  echo [Clutch.GG] Pattern B: legacy 8394 args",
      "  echo CMD: \"League of Legends.exe\" \"8394\" \"LoLLauncher.exe\" \"\" \"spectator %SPEC_HOST% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\"",
      "  start \"\" \"League of Legends.exe\" \"8394\" \"LoLLauncher.exe\" \"\" \"spectator %SPEC_HOST% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%\"",
      "  popd",
      "  timeout /t 8 /nobreak >nul",
      "  for /f \"delims=\" %%C in ('tasklist /FI \"IMAGENAME eq League of Legends.exe\" ^| find /C \"League of Legends.exe\"') do set POST_COUNT=%%C",
      "  echo [Clutch.GG] Post-launch processes: %POST_COUNT%",
      "  if not \"%POST_COUNT%\"==\"%PRE_COUNT%\" goto END_LAUNCED",
      "  pushd \"%LL_DIR%\\Game\"",
      "  echo [Clutch.GG] Pattern A: modern --spectator args (quoted)",
      "  echo CMD: \"League of Legends.exe\" --spectator \"%SPEC_HOST%\" \"%SPEC_KEY%\" \"%SPEC_GAME%\" \"%SPEC_PLATFORM%\" --locale=en_US",
      "  start \"\" \"League of Legends.exe\" --spectator \"%SPEC_HOST%\" \"%SPEC_KEY%\" \"%SPEC_GAME%\" \"%SPEC_PLATFORM%\" --locale=en_US",
      "  popd",
      "  timeout /t 8 /nobreak >nul",
      "  for /f \"delims=\" %%C in ('tasklist /FI \"IMAGENAME eq League of Legends.exe\" ^| find /C \"League of Legends.exe\"') do set POST_COUNT=%%C",
      "  echo [Clutch.GG] Post-launch processes: %POST_COUNT%",
      ...
      ")",
      "",
      "REM ================= Try 2: Launch via LeagueClient.exe (headless spectator) =================",
      "if defined LL_CLIENT (",
      "  echo [Clutch.GG] Launching via LeagueClient.exe...",
      "  pushd \"%LL_DIR%\"",
      "  start \"\" \"LeagueClient.exe\" --spectator %SPEC_HOST% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%",
      "  popd",
      "  timeout /t 10 /nobreak >nul",
      "  tasklist /FI \"IMAGENAME eq League of Legends.exe\" | find /I \"League of Legends.exe\" >nul",
      "  if %ERRORLEVEL%==0 goto END_LAUNCHED",
      ")",
      "",
      "REM ================= Try 3: Fallback via RiotClientServices.exe =================",
      "if defined RC_PATH (",
      "  echo [Clutch.GG] Launching via Riot Client...",
      "  start \"\" \"%RC_PATH%\" --launch-product=league_of_legends --launch-patchline=live --allow-multiple-clients -- --spectator %SPEC_HOST% %SPEC_KEY% %SPEC_GAME% %SPEC_PLATFORM%",
      "  timeout /t 10 /nobreak >nul",
      "  tasklist /FI \"IMAGENAME eq League of Legends.exe\" | find /I \"League of Legends.exe\" >nul",
      "  if %ERRORLEVEL%==0 goto END_LAUNCHED",
      ")",
      "",
      "echo [Clutch.GG] Could not find a valid executable to launch.",
      "",
      ":END",
