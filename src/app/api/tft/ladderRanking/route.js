	...
	try {
		...
		if (ladderRankElement.length > 0) {
			...
			if (anyLadderText.length > 0) {
				ladderRankText = anyLadderText.first().text();
			}
		}
		...
		const pattern1 =
			/TFT Ladder Ranking\s*([0-9,]+)(?:st|nd|rd|th)\s*\(\s*([0-9.]+)%\s*of top\s*\)/i;
		match = ladderRankText.match(pattern1);

		if (!match) {
			// Pattern 2: More flexible pattern with optional spaces
			const pattern2 =
				/TFT Ladder Ranking.*?([0-9,]+)(?:st|nd|rd|th)?.*?\(\s*([0-9.]+)%\s*of top\s*\)/i;
		...
		if (!match) {
			...
			const percentileMatch = ladderRankText.match(
				/\(\s*([0-9.]+)%\s*of top\s*\)/
			);
		...
		if (!match && ladderRankElement.length > 0) {
			...
			const percentileSpan = ladderRankElement.find(
				'span:contains("% of top")'
			);
