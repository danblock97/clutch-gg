...
export default function FeaturedGamesPageContent() {
	...
	useEffect(() => {
		const fetchDDragonData = async () => {
			try {
				...
				// We can continue without champion data, it just won't show images/names
			}
		};
		fetchDDragonData();
	}, []);
		...
		const fetchGames = async () => {
			...
			try {
				const response = await fetch(
					`/api/league/featured-games?region=${region}`
				);
				if (!response.ok) {
				...
				if (data.gameList) {
					const queues = [
						...new Set(data.gameList.map((g) => g.gameQueueConfigId)),
					];
	...
	const filteredGames = games.filter(
		(game) =>
			selectedQueue === "ALL" ||
			game.gameQueueConfigId === parseInt(selectedQueue)
	);
					...
					{filteredGames.length > 0 ? (
						...
						<div className="text-center py-10 col-span-full">
							<p className="text-lg text-[--text-secondary]">
								No featured games available in this region right now.
							</p>
						</div>
					)}
				</div>
