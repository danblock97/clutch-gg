...
const LeaderboardTable = ({ leaderboardData, region, tier }) => {
	...
	const getRowHighlightClass = (index) => {
		if (index === 0) return "bg-gradient-to-r from-yellow-500/20 to-yellow-500/5"; // First place
	...
	const getRankIndicator = (index) => {
		if (index === 0) return <FaCrown className="text-yellow-500 mr-2" title="1st Place" />;
		if (index === 1) return <FaMedal className="text-gray-300 mr-2" title="2nd Place" />;
		if (index === 2) return <FaMedal className="text-amber-700 mr-2" title="3rd Place" />;
		return null;
	};
								...
								<div className="flex flex-col sm:flex-row items-center justify-center gap-2">
									...
									<div className="w-full max-w-[100px] h-5 bg-[--card-bg] rounded-full overflow-hidden relative">
										...
										<div className="absolute inset-0 flex items-center justify-center text-xs font-medium">
											<span className={winrateInfo.colorClass}>{winrateInfo.percent}%</span>
										</div>
									</div>
								</div>
							</div>
