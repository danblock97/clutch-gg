...
import { LuSignalHigh, LuSignalMedium, LuSignalLow, LuTriangleAlert, LuMinus } from "react-icons/lu";
        ...
        <div className="overflow-x-auto pb-6">
            <div className="flex gap-4 min-w-max px-1">
                {teamStates.map((state) => {
                    const group = groupedIssues[state.id];
                    const groupIssues = group?.issues || [];

                    // Optional: Hide "Canceled" or specific states if empty? 
                    // Let's keep it simple and show all for "professional feel"
                        ...
                        <div key={state.id} className="w-80 flex-shrink-0 flex flex-col gap-4">
                            ...
                            <div className="flex flex-col gap-3 min-h-[200px]">
                                {groupIssues.map(issue => (
                                    <div key={issue.id} className="bg-[--card-bg] border border-white/5 p-4 rounded-xl shadow-lg hover:border-white/10 transition group flex flex-col gap-3">
                                        ...
                                        <div className="flex items-center justify-between pt-2 border-t border-white/5 mt-auto">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {/* Labels - moved here if we want or keep above. Let's keep distinct tags above or here? 
                                    The previous design put tags in the middle. Let's put them here next to assignee for compactness 
                                    OR keep them simple. Let's stick them here for now.*/}
